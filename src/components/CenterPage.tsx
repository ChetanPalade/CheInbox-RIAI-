...
const CenterPage: React.FC<Props> = ({ selectedThread }) => {
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(
        `https://hiring.reachinbox.xyz/api/v1/chebox/messages/${selectedThread}`,
        {
          headers: {
            Authorization: token,
          },
        ...
        setSelectedMail([
          {
            id: 0,
            fromName: "",
            fromEmail: "cheBox@icloud.com",
            toName: "",
            toEmail: " cp19@mail.com",
            subject: "New Product Launch",
            body: "I would like to introduce you about ReachInBox and CheBox, a productized design service specifically  for easy messaging. Our aim is to help you enhance the user experience and boost the visual appeal of your software products.y",
            sentAt: "2024-08-22 T12:00:00.000IST",
          },
        ]);
      }
    };
  ...
  return (
    <div className="overflow-y-scroll no-scrollbar  h-full">
      <div className="border-b-2 dark:border-[#33383F] border-[#E0E0E0] w-full flex justify-between px-8 py-4">
        <div>
          <div className="dark:text-white text-black text-lg">Chetan</div>
          <div className="dark:text-[#FFFFFF66] text-[#293642b2] text-sm">cp19@gmail.com</div>
        </div>
        <div className="flex items-center space-x-3">
          <div className="flex dark:bg-[#1F1F1F] bg-white border dark:border-[#343A40] items-center text-black dark:text-white rounded-md py-2 px-3 text-sm">
            <GoDotFill className="text-green-500 text-xl" /> Meeting Completed{" "}
